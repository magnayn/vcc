 ...
 * Pauses computers.
 */
public class StartComputerTask extends AbstractComputerActionTask {
    /**
     * {@inheritDoc}
     */
    protected void recordFailure(String name) {
        log("Computer " + name + " failed to start.", Project.MSG_ERR);
    }
    ...
    protected void recordSuccess(String name) {
        log("Computer " + name + " started.", Project.MSG_INFO);
    }
    ...
    protected Future<Success> doAction(Computer c) {
        if (PowerState.RUNNING.equals(c.getState())) {
            ...
            log("Starting computer " + c.getName() + "...", Project.MSG_INFO);

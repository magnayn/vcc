 ...
 * Pauses computers.
 */
public class StopComputerTask extends AbstractComputerActionTask {
    /**
     * {@inheritDoc}
     */
    protected void recordFailure(String name) {
        log("Computer " + name + " failed to stop.", Project.MSG_ERR);
    }
    ...
    protected void recordSuccess(String name) {
        log("Computer " + name + " stopped.", Project.MSG_INFO);
    }
    ...
    protected Future<Success> doAction(Computer c) {
        if (PowerState.STOPPED.equals(c.getState())) {
            ...
            log("Stopping computer " + c.getName() + "...", Project.MSG_INFO);
